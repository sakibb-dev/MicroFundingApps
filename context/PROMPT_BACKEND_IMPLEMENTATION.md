...
PENTING — Arsitektur auth harus mendukung multi-role login bersamaan di tab browser berbeda (contoh: admin login di tab 1, investor login di tab 2, tanpa saling logout). Ini TIDAK bisa dicapai dengan cookie-based session (Laravel Sanctum SPA mode default) karena cookie di-share ke seluruh browser, bukan per-tab.

Gunakan pendekatan berikut:
- Laravel Sanctum dalam mode **token API**, BUKAN mode SPA/cookie. Saat login, generate personal access token (`$user->createToken(...)`) dan kembalikan sebagai string di response body — jangan di-set sebagai cookie.
- Frontend bertanggung jawab menyimpan token ini di `sessionStorage` (bukan `localStorage` atau cookie) — lihat PROMPT_FRONTEND_IMPLEMENTATION.md bagian auth untuk detail kenapa ini penting.
- Setiap request ter-autentikasi mengirim token via header `Authorization: Bearer {token}`, bukan mengandalkan cookie otomatis.
- Saat logout, revoke token yang sedang dipakai (`$request->user()->currentAccessToken()->delete()`) — jangan invalidate semua token milik user (supaya sesi di tab/device lain tetap jalan).
- Endpoint login harus menerima parameter atau mendeteksi role dari kredensial (investor/umkm/admin login dari form terpisah tapi hit endpoint yang sama atau endpoint per-role — pilih salah satu, sebutkan alasannya) dan mengembalikan token yang scope-nya sesuai role tersebut (gunakan Sanctum ability/scope per token: `createToken('admin-session', ['admin'])`).

- 3 role: investor, umkm, admin — gunakan Laravel Policy atau Spatie Permission untuk otorisasi setelah token diverifikasi
- Middleware per route group memastikan investor tidak bisa akses endpoint UMKM/Admin dan sebaliknya — validasi baik dari `abilities` token maupun role user di database (defense in depth, jangan cuma andalkan salah satu)
